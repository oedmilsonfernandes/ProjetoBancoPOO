...
public class CadastrarConta {
  ...
  public Resultado<Boolean> execute(ContaDTO contaDTO) {
    ...
    Resultado<Boolean> bancoEncontrado =
        _verificarBancoRepository.verSeBancoExiste(contaDTO.getAgencia());
    ...
    switch (contaDTO.getTipoConta()) {
      case ContaBancaria:
        Conta novaContaBancaria =
            ContaFactory.criarContaBancaria(
                contaDTO.getNumero(),
                contaDTO.getAgencia(),
                contaDTO.getCliente(),
                contaDTO.getSaldo());
        ...
        break;
      case ContaEspecial:
        Conta novaContaEspecial =
            ContaFactory.criarContaEspecial(
                contaDTO.getNumero(),
                contaDTO.getAgencia(),
                contaDTO.getCliente(),
                contaDTO.getSaldo(),
                contaDTO.getLimite().get());
        contaCadastrada = this._cadastrarContaRepository.salvar(novaContaEspecial);
        break;
      case ContaPoupanca:
        Conta novaContaPoupanca =
            ContaFactory.criarContaPoupanca(
                contaDTO.getNumero(),
                contaDTO.getAgencia(),
                contaDTO.getCliente(),
                contaDTO.getSaldo(),
                contaDTO.getTaxaJuros().get());
